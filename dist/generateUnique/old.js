...
const generate = input => {
  ...
  var punctuation = () => {
    ...
    if (puncRand() <= 10) return '.';else if (puncRand() <= 11) return '!';else return '?';
  };
  if (Array.isArray(input.charSet)) {
    for (var i = 0; i < input.charCount; i++) {
      if (input.whiteSpace === true && !isEven(i)) string += ' ';else if (input.whiteSpace === true) {
        let glyph = modString();
    ...
    for (var i = 0; i < input.charCount; i++) {
      if (input.whiteSpace === true && !isEven(i)) string += ' ';else string += input.charSet.charAt(randomIndex());
    }
  }
...
const GenerateUnique = input => {
  ...
  if (!input.preset && !input.charSet || typeof input.charSet != 'string' && !Array.isArray(input.charSet)) input.charSet = '23456789abdegjkmnpqrvwxyz';
  var id = '',
    retries = 0,
    uniqueRetries = 9999;
