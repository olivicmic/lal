module.exports = ({ chalk, expect, lal }) => describe('Testing lal.lookupIP', () => {
	...
	it('should return object with status property with \'success\' value', (done) => {
		lal.lookupIP({ ip: testIP }, (err, result) => {
			...
			console.log(chalk.hex('f0f')('http://ip-api.com/json/' + testIP));
			console.log(result);
	...
	it('should return object with country property with \'US\' value', (done) => {
		lal.lookupIP({ ip: testIP, host: 'ipapi' }, (err, result) => {
			if (err) return done(err);
			console.log(chalk.hex('f0f')('https://ipapi.co/' + testIP +'/json/'));
			console.log(result);
	...
	// 		if (err) return done(err);
	// 		console.log(chalk.hex('f0f')('http://extreme-ip-lookup.com/json/' + testIP));
	// 		console.log(result);
	...
	it('should return object with country property with \'US\' value', (done) => {
		lal.lookupIP({ ip: testIP, host: 'ipinfo' }, (err, result) => {
			if (err) return done(err);
			console.log(chalk.hex('f0f')('https://ipinfo.io/' + testIP + '/json'));
			console.log(result);
			expect(result.country).to.equal('US');
			done();
		});
