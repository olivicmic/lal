...
const punctuate = (chance, i, capNext, stringEnd, modString) => {
	let doPeriod = () => random(chance),
		...
		glyph = modString;
	return (i === 0 || capNext === true) ?
		[stringEnd ? punctuation() : glyph.charAt(0).toUpperCase() + glyph.slice(1), false] :
		(addPunc || stringEnd) ? [modString + punctuation(), true] : [modString, false];
};

const generate = ({ charCount, charSet, sentences, whiteSpace }) => {
	let string = '',
		chance = 9,
		capNext = false,
	...
	if (Array.isArray(charSet)) {
		for (let i = 0; i < charCount; i++) {
			if (whiteSpace && !is.even(i)) string += ' ';
			else if (sentences && whiteSpace) {
		...
		for (let i = 0; i < charCount; i++) {
			if (whiteSpace && !is.even(i)) string += ' ';
			else string += charSet.charAt(randomIndex());
		}
	}
...
const GenerateUnique = input => {
	const { charCount = 6, charSet, existing = [], preset, whiteSpace, ...rest } = input || {};
	const newConfig = {
